...
function Leaderboard() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 100; i++) {
      dustParticles.push({
        ...
        color: `hsla(${Math.random() * 60 + 240}, 70%, 70%, ${Math.random() * 0.5 + 0.2})`
      });
    }
    ...
    for (let i = 0; i < 3; i++) {
      galaxies.push({
        ...
        rotationSpeed: (Math.random() * 0.001 + 0.0002) * (Math.random() > 0.5 ? 1 : -1),
    ...
    const drawAurora = () => {
      ...
      for (let x = 0; x < canvas.width; x += 20) {
        const waveHeight = Math.sin((x / canvas.width) * Math.PI * 2 + time) * 20;
        const waveHeight2 = Math.sin((x / canvas.width) * Math.PI * 4 + time * 1.5) * 10;
    ...
    const updateGlows = () => {
      for (let i = 0; i < glowIntensities.length; i++) {
        ...
        if (listItem) {
          ...
          listItem.style.boxShadow = `0 0 ${10 + glowIntensities[i] * 10}px hsla(${hue}, 70%, 50%, ${glowIntensities[i] * 0.7})`;
    ...
    const animate = () => {
      ...
      nebulae.forEach(nebula => {
        ...
        gradient.addColorStop(0, `hsla(${nebula.hue}, 80%, 50%, ${nebula.opacity * 1.5})`);
        gradient.addColorStop(0.5, `hsla(${nebula.hue + 20}, 70%, 40%, ${nebula.opacity})`);
      ...
      stars.forEach(star => {
        ...
        // Make stars twinkle
        star.size = Math.max(1, star.size + (Math.random() * 0.2 - 0.1) * star.speed);
  ...
  useEffect(() => {
    async function fetchScores() {
      ...
      try {
        ...
        // Delayed loading for animation effect
        setTimeout(() => {
  ...
  return (
    <div
      ...
      style={{
        minHeight: '100vh',
        background: 'linear-gradient(135deg, #0a0220 0%, #1b0042 50%, #0a0220 100%)',
        padding: '5rem',
        display: 'flex',
    ...
    >
      ...
      <Container size="sm" py="xl" style={{ position: 'relative', zIndex: 1 }}>
        <Title
          ...
          style={{
            ...
            fontWeight: 'bold',
            color: '#f2e6ff',
            textShadow: '0 0 15px rgba(153, 51, 255, 0.7), 0 0 30px rgba(90, 20, 200, 0.5)',
            fontSize: '2.5rem',
            position: 'relative',
            animation: 'cosmicPulse 4s infinite alternate'
          }}
        >
          <span className="trophy-left" style={{ marginRight: '15px', display: 'inline-block', animation: 'trophyFloat 3s infinite alternate' }}>🏆</span>
          Record of Resumes
          <span className="trophy-right" style={{ marginLeft: '15px', display: 'inline-block', animation: 'trophyFloat 3s infinite alternate-reverse' }}>🏆</span>
        </Title>
        
        {loading ? (
          <Center style={{ height: '300px' }}>
            <div className="cosmic-loader" style={{ position: 'relative' }}>
              <div className="cosmic-loader-ring" style={{
                width: '100px',
                height: '100px',
                borderRadius: '50%',
          ...
          <Paper
            ...
            style={{
              ...
              boxShadow: '0 0 30px rgba(90, 30, 160, 0.5), inset 0 0 20px rgba(180, 120, 255, 0.2)',
          ...
          >
            ...
            <div className="board-shimmer-top" style={{
              position: 'absolute',
              top: 0,
              left: '-100%',
              width: '300%',
            ...
            <div className="board-shimmer-bottom" style={{
              position: 'absolute',
              bottom: 0,
              right: '-100%',
              width: '300%',
            ...
            <List spacing="lg" size="lg" style={{ position: 'relative' }}>
              {leaderboard.map((person, index) => (
                <List.Item
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 15px hsla(${270 + index * 15}, 70%, 50%, 0.4)`,
                  ...
                  icon={
                    <ThemeIcon 
                      color={index === 0 ? 'gold' : index === 1 ? 'silver' : index === 2 ? 'orange' : 'purple'} 
                      size={40} 
                      radius="xl"
                      style={{
                        boxShadow: `0 0 10px ${index === 0 ? 'rgba(255, 215, 0, 0.7)' : index === 1 ? 'rgba(192, 192, 192, 0.7)' : index === 2 ? 'rgba(205, 127, 50, 0.7)' : 'rgba(147, 51, 234, 0.7)'}`,
                ...
                >
                  <div className="item-content" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ 
                      color: '#f2e6ff', 
                      fontWeight: '600',
                      fontSize: '1.1rem', 
      ...
      <style jsx="true">{`
        ...
        @keyframes boardGlow {
          0% { box-shadow: 0 0 30px rgba(90, 30, 160, 0.5), inset 0 0 20px rgba(180, 120, 255, 0.2); }
          50% { box-shadow: 0 0 40px rgba(130, 80, 220, 0.6), inset 0 0 25px rgba(200, 150, 255, 0.3); }
          100% { box-shadow: 0 0 30px rgba(90, 30, 160, 0.5), inset 0 0 20px rgba(180, 120, 255, 0.2); }
        }
        ...
        @keyframes cosmicPulse {
          0% { text-shadow: 0 0 15px rgba(153, 51, 255, 0.7), 0 0 30px rgba(90, 20, 200, 0.5); }
          100% { text-shadow: 0 0 25px rgba(180, 100, 255, 0.9), 0 0 40px rgba(120, 60, 220, 0.7); }
        }
